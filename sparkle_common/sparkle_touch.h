...
{
public:
    sparkle_touch(struct wl_client *client, int version, uint32_t id, were_object_pointer<sparkle_display> display) :
        sparkle_wl_touch(client, version, id), display_(display)
    {
    }

    void down(were_object_pointer<sparkle_surface> surface, int id, int x, int y)
    {
    ...
    void up(were_object_pointer<sparkle_surface> surface, int id, int x, int y)
    {
        send_up(sparkle::next_serial(display_), sparkle::current_msecs(), id);
        send_frame();
    }

    void motion(were_object_pointer<sparkle_surface> surface, int id, int x, int y)
    {
